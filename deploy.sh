...
if [ "$LAMBCI_BRANCH" = "master" ]; then
  export STAGE=prod
  API_STAGE="green"
  EXPLORER_API_PREFIX="explorer-api-$API_STAGE"
else
  export STAGE=dev
...
aws cloudformation deploy --stack-name $STACK_NAME --parameter-overrides "StageParameter=$STAGE" "ExplorerApiStage=$API_STAGE" --template-file $TEMPLATE_FILE || true
